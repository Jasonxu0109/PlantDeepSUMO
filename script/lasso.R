...
for(cv in 10){
  ...
  for (i in 1:length(folds)){
    ...
    train_y <- train$label[-testIndex]
    val_x <- x_train[testIndex,]
    val_y <- train$label[testIndex]
    # 构建和训练模型
    lasso_model <- glmnet(train_x, train_y, family = "binomial", alpha = 1)
    # 计算验证集准确率
    #results <- model %>% evaluate(val_x, val_y, verbose = 0)
    pred <- predict(lasso_model, val_x, type = "response",s=0.01)
    pred <- as.numeric(pred)
    re <- metrics(pred,val_y)
    re <- data.frame(Type='train',re)
    pred <- predict(lasso_model, x_test, type = "response",s=0.01)
    pred <- as.numeric(pred)
    re2 <- metrics(pred,test$label)
    re2 <- data.frame(Type='test',re2)
    result <- rbind(result,re,re2)
  }
}
write.csv(result,"model/evaluation/lasso_evaluation.csv",row.names=F)
#######
result %>%
