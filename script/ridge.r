...
for(cv in 10){
  ...
  for (i in 1:length(folds)){
    ...
    # 构建和训练模型
    ridge_model <- glmnet(train_x, train_y, family = "binomial", alpha = 0)
    # 计算验证集准确率
    #results <- model %>% evaluate(val_x, val_y, verbose = 0)
    pred <- predict(ridge_model, val_x, type = "response",s=0.01)
    pred <- as.numeric(pred)
    re <- metrics(pred,val_y)
    re <- data.frame(Type='train',re)
    pred <- predict(ridge_model, x_test, type = "response",s=0.01)
    pred <- as.numeric(pred)
...
write.csv(result,"model/evaluation/ridge_evaluation.csv",row.names=F)
#######
result %>%
