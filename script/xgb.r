...
x_test <- xgb.DMatrix(data = x_test, label = test$label)
...
for(cv in 10){
  ...
  for (i in 1:length(folds)){
    ...
    val_x <- xgb.DMatrix(data = val_x, label = val_y)
    # 构建和训练模型
    xgb_model <- xgb.train(
      params = params,  # 参数
      data = train_x,  # 训练数据
...
write.csv(result,"model/evaluation/xgb_evaluation.csv",row.names=F)
#######
result %>%
